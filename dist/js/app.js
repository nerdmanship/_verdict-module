...
var initAnimation = function initAnimation() {
  ...
  var logoPos = '820 820 950 950'; // check
  var arenaPos = '400 180 1800 1800';
  var fightPos = '800 370 1200 1200';
  var clockPos = '720 400 1150 1150'; // check
  var followPos = '600 350 1200 1200';
  var heroPos = '500 700 1600 1600';
  var interfacePos = '650 950 1150 1150';
  var featuresPos = '1170 1400 100 100';
  ...
  // Main
  tl.add(reset).add(tlCam.play(), 0).add(tlTitle.play(), 0).add(tlArena.play(), 3).add(tlHero.play(), 15);

  // Sub-Level 1
  tlCam.set(view, { attr: { viewBox: logoPos } }).to(view, 3, { rotation: -1, transformOrigin: "center", attr: { viewBox: arenaPos }, ease: Power3.easeInOut }, 4).to(view, 2.7, { rotation: 1, transformOrigin: "center", attr: { viewBox: fightPos }, ease: Power1.easeInOut }, 7).to(view, 1.3, { rotation: 0, transformOrigin: "center", attr: { viewBox: clockPos }, ease: Power4.easeInOut }, 9.7).to(view, 2.5, { rotation: 1, transformOrigin: "center", attr: { viewBox: followPos }, ease: Power1.easeInOut }, 11).to(view, 2.5, { rotation: 0, transformOrigin: "center", attr: { viewBox: heroPos }, ease: Power2.easeInOut }, 13.5).to(view, 1, { rotation: 3, transformOrigin: "center", attr: { viewBox: interfacePos }, ease: Power3.easeInOut }, 19)

  // zoom in to white and fade out everything
  .to(view, 2, { rotation: 0, transformOrigin: "center", attr: { viewBox: featuresPos }, ease: Power4.easeInOut }, 26).set(view, { attr: { viewBox: logoPos }, ease: Power4.easeInOut }, 28);

  tlTitle.add("title").add(tlLogo.play(), "title").add(tlTagline.play(), "title =+1.5");

  tlArena.add("arena").add(tlCage.play(), "arena").add(tlCountdown.play(), "arena =+2").add(tlFight.play(), "arena =+2").add(tlWalk.play(), "arena =+8");
  ...
  tlFight.fromTo(fighters, 0.3, { x: -280 }, { x: -300 });

  tlWalk.to(fighters, 10, { x: -700, ease: Power1.easeNone });
  ...
  tlLeader.set(leader, { x: 240 }).set(leaderContainer, { autoAlpha: 1 }).set(you, { y: 275 }).set(player[0], { y: 235 }).set(player[1], { y: 195 }).set(player[2], { y: 155 }).to(leader, 0.3, { x: 0 }).staggerFromTo(xp, 2.5, { y: 70, scale: 0, autoAlpha: 1, transformOrigin: "center" }, { y: -50, scale: 5, autoAlpha: 0 }, -0.2, 0.5).to(xp[0], 2.5, { bezier: { curviness: 2, values: [{ x: 0 }, { x: 20 }, { x: -20 }, { x: 20 }] } }, 0.5).to(xp[1], 2.5, { bezier: { curviness: 2, values: [{ x: 0 }, { x: -10 }, { x: 30 }, { x: -20 }] } }, 0.5).to(xp[2], 2.5, { bezier: { curviness: 2, values: [{ x: 0 }, { x: 30 }, { x: -10 }, { x: 30 }] } }, 0.5).to(you, 0.7, { y: 153, ease: Back.easeInOut }, 1).staggerTo(player, 0.3, { y: "+=40", ease: Power3.easeOut }, 0.1, 1.2);

  tlFeatures.set(features, { x: 240 }).set(bullets[0], { y: 1200 }).set(bullets[1], { y: 1250 }).set(bullets[2], { y: 1300 }).set([featuresContainer, bulletList], { autoAlpha: 1 }).to(features, 0.3, { x: 0 }).set([cage, hero, logoGradient, logoShadow, logo, tagline], { autoAlpha: 0 }, 1).staggerFrom(bullets, 1, { x: 2000, ease: Power4.easeInOut }, 0.7, 1).staggerTo(bullets, 1, { x: 0, ease: Power4.easeInOut }, 0.3, 5);
